...
const components = [
  ...
  },
  {
    ...
    ring: "ring-emerald-safe/20",
    bg: "bg-emerald-safe/8",
  },
  {
    icon: Cpu,
    label: "Path Service",
...
export default function Architecture() {
  return (
    <section
      id="architecture"
      className="py-28 section-surface overflow-hidden"
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="text-center mb-14">
          ...
          >
            SafeTrail isn&apos;t just an app — it&apos;s a multi-layer safety
            ecosystem. A Node.js core, distributed Python microservices for path
            analysis, and a Polygon blockchain layer for immutable audit trails,
        ...
        >
          ...
          <div className="absolute bottom-4 right-4 flex items-center gap-2 px-4 py-2 bg-white/90 border border-beige-200 rounded-full shadow-md backdrop-blur-sm">
            ...
            <span className="text-[11px] font-bold uppercase tracking-widest text-beige-700">
              System Architecture
            </span>
          </div>
        </motion.a>
        ...
        >
          {components.map((c, i) => (
            ...
            >
              <div
                className={`w-11 h-11 rounded-xl flex items-center justify-center ring-1 ${c.ring} ${c.bg} group-hover:scale-110 transition-transform`}
              >
                <c.icon className={`w-5 h-5 ${c.color}`} />
              </div>
              <div className="text-center">
                <p className="text-xs font-bold text-beige-900 leading-tight">
                  {c.label}
                </p>
            ...
            @media (min-width: 640px) {
              ...
              .comp-row::before {
                ...
                background: linear-gradient(
                  90deg,
                  transparent,
                  rgba(168, 130, 90, 0.2) 20%,
                  rgba(168, 130, 90, 0.2) 80%,
                  transparent
                );
                z-index: 0;
              }
        ...
        <p className="text-center text-xs text-beige-400 mt-6">
          5 interconnected services · Real-time data flow via Socket.IO ·
